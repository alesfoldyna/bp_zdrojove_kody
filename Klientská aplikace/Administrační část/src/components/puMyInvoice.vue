            ...
            <div class="row h-100">
                ...
                <main class="col">
                    <div class="container mt-3">
                        <h1 class="font-weight-light">Moje faktury</h1>
                        ...
                        <table-for-invoice v-else :items="myIvoices"></table-for-invoice>
                    </div>
                </main>
            </div>
        </div>
...
// Komponenta pro výpis seznamu faktur profesionálnímu prodejci.
export default {
  name: 'puMyInvoice',
  data () {
    return {
  ...
  methods:{
        /**
         * @vuese
         * Požadavek do API na získání seznamu faktur uživatele
         */
        init(){
            let param = {
            ...
            this.$http.post(process.env.API_URL+'v1/sl/token', param).then(function(response){
                this.$http.get(process.env.API_URL+'v1/sl/my/'+this.$root.user.id_user+'/invoices', {
                    headers: {
                    'Authorization': 'Bearer '+ response.body.token,
                    'Accept': 'application/json'
                    }
                }).then(function(response){
                    this.myIvoices = response.body;
                    this.isBusy = false;
                }, function(response){
