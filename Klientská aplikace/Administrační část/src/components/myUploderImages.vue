<template>
  <div class="UploadCard card bg-light w-25 p-3">
    <span v-show="imagePreview" class="rubbish-icon" v-on:click="onDelete" :alt="'delete'+id"></span>
    <img :src="(setImagePreview === null) ? imagePreview : imagePreview.dataUrl" v-if="imagePreview" class="img-preview w-100"/> 
    <img src="@/icons/image.png" v-else class="img-preview w-100"/> 

    <input :id="id" :class="className" style="display: none" type="file" @change="uploadFile" :accept="accept"/>
    <label :for="id" slot="upload-label" class="inputButton btn btn-primary mt-3 w-100">
        <figure>
          <svg
        ...
        <span class="upload-caption">{{imagePreview ? "Replace" : "Click to upload"}}</span>
    </label>
  </div>
</template>
...
// Upravená komponenta knihovny vue-image-upload-resize
export default {
  ...
  methods: {
    onDelete(){
        this.$emit('onDelete', this.indexOfArr);
    },  
    /**
     * Get file from input
     * @param  {object} event
  ...
  created() {
    ...
    console.log(this.setImagePreview)
    if(this.setImagePreview !== null){
...
<style scoped>
    ...
    span.rubbish-icon {
        background: url("../icons/rubbish-bin.png");
    ...
    span.rubbish-icon:hover{
        background: url("../icons/rubbish-bin-select.png");
        background-size:cover;
    }
</style>
