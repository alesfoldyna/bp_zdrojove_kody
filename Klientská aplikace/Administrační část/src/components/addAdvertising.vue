  ...
  <div class="container-fluid h-100">
    <div class="row h-100">
      ...
      <div class="container mt-3">
        <h1 class="font-weight-light">Přidání inzerátu</h1>
        <b-alert :show="alarm.isAlarm" :variant="alarm.type">{{alarm.AlarmText}}</b-alert>
        <h4 class="font-weight-light text-center text-danger" v-if="countLimit">Máte již vyčerpaný počet vložených inzerátů</h4>
          <form v-else v-on:submit.prevent="addAdvertising">
                  ...
                  <div class="card-body">
                    <div class="row">
                      <div class="form-group col-md">
                        ...
                        <select class="custom-select my-1 mr-2" id="selectZnacka" v-model="inputs.znacka" v-on:change="loadModels(inputs.znacka), inputs.model = null, setModelsMadeYears(), inputs.vybava = [], select.vybava = null" required>
                      ...
                      <div class="form-group col-md">
                        ...
                        <select class="custom-select my-1 mr-2" id="selectModel" :disabled="inputs.znacka == null" v-model="inputs.model" v-on:change="setModelsMadeYears(), loadVybava(), inputs.vybava = [], select.vybava = null" required>
                          <option :value="null">Nevybrano</option>
            ...
            <button class="btn btn btn-primary btn-lg btn-block mt-3 mb-3"  type="submit" >Přidat inzerát</button>
          </form>
...
// Komponenta pro vložení nového inzerátu
export default {
  name: 'addAdvertising',
  data () {
    return {
   ...
   */
    getDateProvozuOd(){
      if(this.inputs.provozu_od.mesic === null || this.inputs.provozu_od.rok === null) {
        ...
        console.log(this.inputs.provozu_od.date)
      }
    },
    /**
   * @vuese
   ...
   */
    ...
    /**
   * @vuese
   * Pošle požadavek do API na přidání inzerátu
   */
    addAdvertising(){
      ...
      let prepareParamForm = {
        ...
        cena_bez_dph: (this.inputs.cena_bez_dph !== null) ? this.inputs.cena_bez_dph : null,
      ...
      this.$http.post(process.env.API_URL+'v1/fl/addNewAdv', prepareParamForm, {
                headers: {
                'Authorization': 'Bearer '+this.$root.user.token,
                'Accept': 'application/json'
                }
            }).then(function(response){
                this.$router.push({ name: 'AdminHome', params: { isAlarm: true, AlarmText: 'Inzerát byl úspěšně vytvořen', alarmStyle: 'success'} });
            }, function(response){
                if(response.status === 500){
                    this.alarm.isAlarm = true;
                    this.alarm.AlarmText = (parseInt(response.body.error.code) === 23505) ? 'Již je vložený inzerát se stejným VIN. NELZE VLOŽIT!' : response.body.error.text;
   ...
   */
    ...
    /**
   * @vuese
   * Zabraňuje profesionálnímu prodejci vložit inzerát, pokudpřekročil svůj limit
   */
    checkCountLimit(){
      this.$http.get(process.env.API_URL+'v1/fl/adv/count/'+this.$root.user.id_user,{
                ...
                if(parseInt(response.body.pocet) >=  this.$root.user.count_limit){
                   this.countLimit = true;
                } else  this.countLimit = false;
            }, function(response){
                if(response.status === 500){
                    this.alarm.isAlarm = true;
  ...
  created: function(){
    ...
    //check for PU count of adv
    if(this.$root.user.user_type === 'PROFI'){
        this.checkCountLimit();
    }
    // init function
    this.loadZnacka();
